"""
Streamlit app to visualize Weave traces for memory strategy analysis.

Usage: uv run streamlit run tools/trace_viewer.py
"""
...
def get_memory_key(call):
    """Extract memory_key from call inputs."""
    inputs = unwrap(call.inputs)
    return (
        inputs.get("memory_key", "unknown") if isinstance(inputs, dict) else "unknown"
    )
...
call_id = st.sidebar.text_input(
    "Call ID", placeholder="019b2bfb-3ec6-7bf9-bc1b-1c6829cf5d13"
)
...
try:
    ...
    if not call:
        ...
        st.stop()
except (weave.errors.WeaveError, ValueError, ConnectionError) as e:
...
strategy = get_memory_key(call)
latency = summary.get("weave", {}).get("latency_ms", 0)
memory_key = inputs.get("memory_key", "N/A") if isinstance(inputs, dict) else "N/A"
...
def render_message(msg, idx=None):
    ...
    icons = {"user": "U", "assistant": "A", "tool": "T", "system": "S"}
...
with st.expander("Raw Trace Data"):
    st.json(
        {"trace_id": call.id, "inputs": inputs, "output": output, "summary": summary},
        expanded=False,
    )
