...
def apply_ace_strategy(
    ...
    if has_reasoning:
        ...
        logger.debug(
            f"Reflector output - reflection_text (first 200 chars): {reflection_text[:200] if reflection_text else '<empty>'}..."
        )
        ...
        if bullet_tags:
            ...
            logger.debug(
                "⚠ No bullet_tags returned from Reflector - playbook counts NOT updated"
            )
        ...
        logger.debug(
            f"✗ Reflector SKIPPED (has_reasoning={has_reasoning}, has_bullets={has_bullets})"
        )
    ...
    logger.debug(
        f"Curator conditions: step={state.step_count}, frequency={curator_frequency}, frequency_match={frequency_match}, has_reflection={has_reflection}"
    )
    ...
    if frequency_match:
        ...
        if operations:
            ...
            logger.debug(
                f"Playbook after curation (first 300 chars): {state.playbook[:300]}..."
            )
        else:
    ...
    logger.debug(
        f"Generator output - reasoning_trace (first 200 chars): {reasoning_trace[:200] if reasoning_trace else '<empty>'}..."
    )
