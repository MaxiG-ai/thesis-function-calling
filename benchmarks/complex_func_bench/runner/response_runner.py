...
class RespEvalRunner:
    ...
    def completeness_eval(self, **kwargs):
        complete_result = self.model(
            complete_system_prompt, complete_user_prompt, **kwargs
        )
        ...
        if (
            not isinstance(decoded_complete_result, dict)
            or "score" not in decoded_complete_result
        ):
            return None
    ...
    def correctness_eval(self, **kwargs):
        correct_result = self.model(
            correct_system_prompt, correct_user_prompt, **kwargs
        )
        decoded_correct_result = decode_json(correct_result)
        self.logger.info(f"Correct Result: {decoded_correct_result}")
        if (
            not isinstance(decoded_correct_result, dict)
            or "score" not in decoded_correct_result
        ):
            return None
    ...
    def run(self, data, gen_response):
        if gen_response == "":
            return {
                "complete": {
                    "score": -2,
                    "reason": "Do not generate response successfully.",
                },
                "correct": {
                    "score": -2,
                    "reason": "Do not generate response successfully.",
        ...
        if complete_result and correct_result:
            return {
                "complete": {
                    ...
                    "reason": complete_result.get("reason", None),
                },
                "correct": {
                    "score": correct_result["score"],
                    "reason": correct_result.get("reason", None),
                },
            }
