...
class ModelRunner:
    def __init__(self, args, compare_class=None):
        ...
        # Accept a pre-built CompareFC to avoid reloading FlagModel per case
        self.CompareClass = (
            compare_class if compare_class is not None else CompareFC(args, logger)
        )
    ...
    def only_free_function(self, temp_fcs):
        for call in temp_fcs:
            if (
                ...
                and call["arguments"]["search_type"] == "hotel"
            ):
                return True
    ...
    def init_golden(self, convs):
        ...
        assert len(self.fc_chain) == len(self.obs_chain), (
            "function call and observation length mismatch."
        )

        self.turn_id, self.correct_count = 0, 0
        self.golden_fcs, self.golden_obs = (
            copy.deepcopy(self.fc_chain[self.turn_id]),
            copy.deepcopy(self.obs_chain[self.turn_id]),
        )
    ...
    def return_result(self, messages, error_info=None):
        ...
        if self.turn_id < len(self.fc_chain) or len(self.golden_fcs) > 0:
            self.logger.info(
                f"turn id  = {self.turn_id}; len(golden_answer) = {len(self.fc_chain)}"
            )
            self.logger.info(f"golden_function_calls = {self.golden_fcs}")
            return self.return_result(
                messages, {"error_type": "stop_early", "content": "Stop early."}
            )
